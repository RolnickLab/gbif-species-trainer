...
About          : Calculates macro accuracy of a batch for different taxon levels for different top 'n' accuracies
"""
        ...
        for taxon in new_data.keys():
            ...
            for key in tax_data.keys():
                if key not in prev_data[taxon].keys():
                    ...
                    prev_data[taxon][key] = list(
                        map(add, prev_data[taxon][key], new_data[taxon][key])
                    )

    return prev_data


def final_macro_accuracy(data):
    """
    ...
    for taxon in data.keys():
        tax_data = data[taxon]
        final_accuracy[taxon] = {}
        for key in tax_data.keys():
            final_accuracy[taxon][key] = [
                round(data[taxon][key][0] / data[taxon][key][3] * 100, 2),
    ...
    for taxon in final_accuracy.keys():
        tax_data = final_accuracy[taxon]
        top1_tot = 0
        top3_tot = 0
        top10_tot = 0
        for key in tax_data.keys():
            top1_tot += final_accuracy[taxon][key][0]
            top3_tot += final_accuracy[taxon][key][1]
            top10_tot += final_accuracy[taxon][key][2]
